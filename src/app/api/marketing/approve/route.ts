...
export async function GET(req: NextRequest) {
  ...
  const { data: post, error } = await supabase
    ...
    .select('id, approval_status, platform, scheduled_for')
    .eq('approval_token', token)
    .single()

  if (error || !post) {
    return page('Not found', '🔍', 'Post not found', 'This approval link is invalid or has expired.', APP_URL, 'Go to ContentAI')
  }

  if (post.approval_status === 'approved') {
    return page('Already approved', '✅', 'Already approved', 'This post was already approved and is queued for publishing.', APP_URL, 'Go to ContentAI')
  }

  if (post.approval_status === 'skipped') {
    return page('Post skipped', '⏭', 'Post was skipped', 'This post has already been skipped.', APP_URL, 'Go to ContentAI')
  }
  ...
  if (updateErr) {
    return page('Error', '❌', 'Could not approve', `Database error: ${updateErr.message}`, APP_URL, 'Go to ContentAI')
  }

  const platform = post.platform ?? 'post'
  const when = post.scheduled_for
    ? new Date(post.scheduled_for).toLocaleString('en-CA', {
  ...
  return page(
    ...
    '✅',
    'Post approved!',
    `Your ${platform} post is approved and will be published automatically at ${when}.`,
    APP_URL,
    'Go to ContentAI'
  )
}
